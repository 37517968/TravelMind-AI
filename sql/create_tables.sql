...
CREATE TABLE IF NOT EXISTS `travel_plan` (
    ...
    `travelType` VARCHAR(64) NULL DEFAULT NULL COMMENT '旅行类型：亲子游/情侣游/朋友游/独自游/家庭游',
...
CREATE TABLE IF NOT EXISTS `travel_comment` (
    ...
    `parentId` BIGINT NULL DEFAULT NULL COMMENT '父评论ID（用于回复功能）',
...
CREATE TABLE IF NOT EXISTS `user_like` (
    ...
    `targetType` VARCHAR(32) NOT NULL COMMENT '目标类型：plan/comment',
    `createTime` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `isDelete` TINYINT NOT NULL DEFAULT 0 COMMENT '是否删除',
    PRIMARY KEY (`id`),
...
VALUES ('admin', 'abcb55ab80d49d3ba8c638e055caff1f', '管理员', 'admin')
ON DUPLICATE KEY UPDATE `userAccount` = `userAccount`;

-- 插入测试普通用户（密码：123456）
INSERT INTO `user` (`userAccount`, `userPassword`, `userName`, `userRole`) 
VALUES ('testuser', 'abcb55ab80d49d3ba8c638e055caff1f', '测试用户', 'user')
ON DUPLICATE KEY UPDATE `userAccount` = `userAccount`;
