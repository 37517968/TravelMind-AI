...
flowchart TD
    ...
    INTENT -->|CONTINUE / NEW_PLAN| EXTRACT[CONSTRAINT_EXTRACTION\nLLM JSON + deterministic fallback]
...
`INTENT_ROUTING` 是图的入口节点，用一次 LLM 结构化调用把输入分成三类：
...
- `CONTINUE`：对当前行程追问的补充或修改（目的地、日期、天数、人数、预算、偏好等），继续正式规划链路。
- `NEW_PLAN`：用户明确改主意或要求另起一段行程。引擎在该分支丢弃由旧检查点恢复出来的行程状态，
  并把请求重置为最新一句诉求（移除上一轮 `supplementalHistory` 合并进来的字段），避免旧行程污染新一轮规划。

上下文与可靠性约束：

- 只拼接 `conversationHistory` 最近 6 轮、每轮裁剪 180 字，避免历史长行程挤占提示词预算；
- 上下文与用户输入一律声明为不可信材料，模型只允许返回 `{"intent","confidence","reason"}` 三字段 JSON；
- 模型不可用、超时或输出不可解析时按关键词规则兜底（重新开始 → `NEW_PLAN`，旅行关键词 → `CONTINUE`，
  问候闲聊 → `CHAT`，等待补充期间的短输入 → `CONTINUE`），规则结论带 `intentSource=HEURISTIC` 便于排障；
- 空输入不调用模型，直接交给 `CONSTRAINT_VALIDATION` 追问，保持零模型消耗；
- 客户端显式声明的 `taskType` 优先于模型判定：`QA` 直接走闲聊分支，`MODIFY` 必然继续规划。
