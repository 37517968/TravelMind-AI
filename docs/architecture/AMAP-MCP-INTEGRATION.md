...
- 高德地图调用使用独立的用户级 Sentinel 配额 `USER_MAP_TOOL_QPS`（默认 20 QPS），不再与普通工具的 5 QPS 共享窗口；发生限流时重试会等待越过当前秒级窗口。
- POI 详情与每一段路线使用独立 SSE 事件键，失败事件包含 `errorCode`，避免一次失败覆盖同类工具之前的成功进度。
